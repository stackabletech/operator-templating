...
class Crate:
  ...
  def bump_level(cls, version, level):
    ...
    return str(ver.bump_prerelease("nightly"))[:-2]  # remove the .1 suffix that semver always adds to the prererelease.
...
def load(root):
  ...
  if "workspace" in ctl:
    return Workspace([load(f"{root}/{path}") for path in ctl["workspace"]["members"]])
